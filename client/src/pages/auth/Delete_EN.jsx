...
export default function Delete_EN({ data_en }) {
  const navigate = useNavigate();
  const [copydata, setCopydata] = useState([...data_en]);

  useEffect(() => {
    axios.get("/api/portfolio_en").then((res) => {
  ...
  const deletePortfolio = (idx) => {
    axios
      .post("/api/deletePortfolio_en", { idx })
      .then(() => {
        console.log("SUCCESS");
        alert("삭제 되었습니다!");
      })
                  ...
                  {copydata.map((copydata, i) => (
                    <TableRow hover key={i}>
                      ...
                      <TableCell
                        ...
                        onClick={() => {
                          navigate(`/auth/modify_en/${copydata.idx}`, {
