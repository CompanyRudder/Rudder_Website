...
export default function Modify_EN() {
  const { id } = useParams();
  const location = useLocation();
  const navigate = useNavigate();

  const PortfolioModifyFormEN = Yup.object().shape({
  ...
  const methods = useForm({
    resolver: yupResolver(PortfolioModifyFormEN),
    defaultValues,
  });
  const {
    reset,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      //--DB UPDATE
      axios
        .post("/api/updatePortfolio_en", { data })
        .then(() => {
          console.log("SUCCESS");
          alert("수정되었습니다!");
          // navigate("/");
